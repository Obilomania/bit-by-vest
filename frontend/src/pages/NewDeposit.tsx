...
const NewDeposit = () => {
  ...
  const thePlansAfterDeposit = thePlans.filter((p: any) => p.timeFrame !== "Beginner");

  const [iAmount, setIAmount] = useState<any>("");
  const allUserTransaction = useSelector((state:any) => state.persistedReducer.transaction)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res:any = await createNewDeposit({
      amount: iAmount,
      plan: selectPlan,
          ...
          <div className="deposit-content">
            <div className="plans">
              {userDepositList.length === 0 ? (
                ...
                <>
                  {thePlansAfterDeposit?.map((data: any) => (
                    <div className="the-plan-card" key={data.id}>
                      <p className="plan-title">{data.timeFrame}</p>
                      <p className="plan-timeframe">{data.returns}</p>
                      <p className="plan-range">{data.range}</p>
